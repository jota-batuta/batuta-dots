...
test_claude_md_is_direct_copy() {
    # GUARDRAIL: BatutaClaude/CLAUDE.md is the tracked canonical source.
    # Root CLAUDE.md is a gitignored mirror regenerated by setup.sh --claude.
    # If this test fails, run: bash infra/setup.sh --claude
    log_test "Root CLAUDE.md mirrors BatutaClaude/CLAUDE.md byte-for-byte"
    [[ ! -f "$REPO_ROOT/CLAUDE.md" ]] && bash "$SETUP_SCRIPT" --claude >/dev/null 2>&1

    if diff -q "$REPO_ROOT/BatutaClaude/CLAUDE.md" "$REPO_ROOT/CLAUDE.md" >/dev/null 2>&1; then
        log_pass "Root CLAUDE.md is identical to BatutaClaude/CLAUDE.md"
    else
        log_fail "Root CLAUDE.md drifted from BatutaClaude/CLAUDE.md — run: bash infra/setup.sh --claude"
    fi
}

test_gemini_md_is_direct_copy() {
    # GUARDRAIL: BatutaAntigravity/GEMINI.md is the tracked canonical source,
    # symmetric with BatutaClaude/CLAUDE.md. Root GEMINI.md is gitignored.
    log_test "Root GEMINI.md mirrors BatutaAntigravity/GEMINI.md byte-for-byte"

    if [[ ! -f "$REPO_ROOT/BatutaAntigravity/GEMINI.md" ]]; then
        log_fail "BatutaAntigravity/GEMINI.md canonical source missing"
        return
    fi

    # Root GEMINI.md may not exist (gitignored, regenerated on demand).
    # Only fail if it exists AND differs from canonical.
    if [[ ! -f "$REPO_ROOT/GEMINI.md" ]]; then
        log_pass "Root GEMINI.md not yet generated (expected — gitignored)"
        return
    fi

    if diff -q "$REPO_ROOT/BatutaAntigravity/GEMINI.md" "$REPO_ROOT/GEMINI.md" >/dev/null 2>&1; then
        log_pass "Root GEMINI.md is identical to BatutaAntigravity/GEMINI.md"
    else
        log_fail "Root GEMINI.md drifted from BatutaAntigravity/GEMINI.md"
    fi
}

test_subagent_hook_exists() {
    # GUARDRAIL: The SubagentStop hook script must exist and be syntactically
    # valid. It is wired in BatutaClaude/settings.json and responsible for
    # appending sub-agent reports to .batuta/team-history.md.
    log_test "subagent-save.sh hook exists and is syntactically valid"
    local hook="$REPO_ROOT/infra/hooks/subagent-save.sh"

    if [[ ! -f "$hook" ]]; then
        log_fail "subagent-save.sh not found at infra/hooks/"
        return
    fi

    if bash -n "$hook" 2>/dev/null; then
        log_pass "subagent-save.sh has valid bash syntax"
    else
        log_fail "subagent-save.sh has syntax errors"
    fi
}

test_subagent_hook_wired_in_settings() {
    # GUARDRAIL: BatutaClaude/settings.json must declare the SubagentStop hook.
    # Without this entry, the hook script is never invoked by Claude Code.
    log_test "BatutaClaude/settings.json wires SubagentStop hook"
    local settings="$REPO_ROOT/BatutaClaude/settings.json"

    if grep -q '"SubagentStop"' "$settings" 2>/dev/null && \
       grep -q 'subagent-save.sh' "$settings" 2>/dev/null; then
        log_pass "SubagentStop hook is wired in settings.json"
    else
        log_fail "SubagentStop hook missing from settings.json hooks block"
    fi
}
...
test_skill_count_total() {
    ...
    if [[ $skill_count -ge $min_skills ]]; then
        log_pass "$skill_count skills (floor $min_skills)"
    else
        log_fail "Found $skill_count skills, expected at least $min_skills (skills may have been accidentally deleted)"
    fi
}
...
test_all_skills_have_purpose_section() {
    log_test "All 33 skills have ## Purpose section (v12.2)"
    local skills_dir="$REPO_ROOT/BatutaClaude/skills"
    local missing=0
