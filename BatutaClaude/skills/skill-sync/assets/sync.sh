...
main() {
    ...
    # --- Step 2: Update scope agent files ---
    for scope in $(echo "${!SCOPES_FOUND[@]}" | tr ' ' '\n' | LC_ALL=C sort); do
        ...
        log_info "Updating agent: ${scope}-agent.md"
        local agent_table
    ...
    if [[ -f "$claude_md" ]]; then
        ...
        local claude_table
        claude_table="$(generate_claude_table ALL_SKILLS)"

        if replace_section "$claude_md" "$claude_table"; then
            log_success "  Updated CLAUDE.md Available Skills table"
        fi
    else
