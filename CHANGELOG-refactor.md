...
As the skill ecosystem grows (22 today, potentially 100+), injecting ALL global skills into every session creates context noise — the agent sees compliance-colombia in a Node project, data-pipeline in a landing page. This release introduces Project Skill Provisioning: during `/sdd-init`, only relevant skills and MCPs are copied to the project. The agent's context stays clean and focused.

### New Feature: Project Skill Provisioning (Auto-Scope)

- **sdd-init Step 3.8** — New provisioning phase that detects project technologies, copies matching skills from `~/.claude/skills/` to `.claude/skills/`, and generates project-specific `.mcp.json`.
- **skill-provisions.yaml** — New data file mapping tech signals to skills and MCPs (`BatutaClaude/skills/sdd-init/assets/skill-provisions.yaml`).
- **`.provisions.json` manifest** — Written to `.claude/skills/.provisions.json` to track what was provisioned, when, and why.

### session-start.sh: 3-Way Skill Detection

- **Provisioned project** (`.provisions.json` exists) — scans ONLY `.claude/skills/` (project-local). Global skills excluded because provisioning already curated the relevant set.
- **Manual skills** (`.claude/skills/` exists, no manifest) — scans both local and global (backward compatible).
- **No local skills** — falls back to `~/.claude/skills/` (backward compatible).

### Antigravity: Project Skill Creation

- **GEMINI.md** — New `/skill <name>` command: creates lightweight project-local skills in `.claude/skills/` during brainstorming/prototyping. Minimal structure (Purpose + Rules), no full ecosystem-creator rigor. Updates `.provisions.json` if present.

### Reprovisioning via sdd-explore

- **sdd-explore Step 2.5** — Enhanced Skill Gap Detection now checks global library first. If a needed skill exists in `~/.claude/skills/` but not in the project, offers to copy it (instead of only creating new).
- **ecosystem-creator Step 6.5** — New skills can register detection rules in `skill-provisions.yaml` for future auto-provisioning.

### Documentation

- **CLAUDE.md** — Added "Project Skill Provisioning (auto-scope)" section.
- **setup_test.sh** — 3 new tests: provisions.yaml exists, has required sections, session-start.sh has 3-way detection.
- **setup_test.sh** — Fixed 3 pre-existing test failures from v11.2 cleanup: updated "Scope Routing" assertion, rewrote `test_orta_hooks_exist` to verify removal, rewrote `test_hooks_point_to_correct_directory` for actual hook paths.

### Principio de diseno

- **Global library = Hub** (`~/.claude/skills/`): All skills available for any project.
- **Project scope = Spoke** (`.claude/skills/`): Only relevant skills, clean context.
- **Reprovisioning path**: `sdd-explore` gap detection can pull from global library anytime.
- **Backward compatible**: Existing projects without `.claude/skills/` work exactly as before.

### Rollback

```bash
git revert <commit-hash>  # Revert v11.3 commit
# session-start.sh will fall back to scanning global (pre-v11.3 behavior)
```

---

## v11.2.0 — Antigravity Lite + Zombie Cleanup (2026-02-26)

### Contexto

Reposition BatutaAntigravity as a lightweight brainstorming and quick prototyping companion.
Claude Code handles production-grade SDD; Antigravity handles exploration and rapid iteration.
A 4-agent audit team analyzed the ecosystem and found 7 critical, 10 high, and 7 medium issues.
This release resolves all 5 critical zombies, 4 high doc issues, and 3 critical script issues.

### Breaking Changes

- **BatutaAntigravity/GEMINI.md** — Complete rewrite as "Antigravity Lite": simplified for brainstorming & prototyping.
- **Eliminated orphan hooks** — Removed `infra/hooks/orta-task-gate.sh` and `infra/hooks/orta-teammate-idle.sh` (dead since v11.1).

### Zombie Cleanup (audit findings)

- **sdd-init/SKILL.md** — Removed PreToolUse hook generation from project config templates (hook eliminated in v11.1).
- **team-orchestrator/SKILL.md** — Removed references to TeammateIdle/TaskCompleted hooks and unmeasurable metrics.
- **setup.sh** — Removed "AUTO-GENERATED by skill-sync" verification (skill-sync eliminated in v11.1).
- **setup_test.sh** — Removed test cases for eliminated features (skill-sync, prompt-tracker, analyze-prompts, dead hooks).

### Script Fixes

- **session-start.sh** — Fixed `date -d` for macOS compatibility (now uses `date -j` on BSD).
- **session-start.sh** — Removed dead "Last updated" field lookup (field was never generated).
- **session-start.sh** — Fixed VERSION lookup: now uses relative paths instead of hardcoded `/tmp/batuta-dots`.

### Documentation Updates

- **README.md / README.es.md** — Updated Antigravity description as Lite companion.
- **GEMINI.md** — Rewritten: lightweight Quick Development Flow replaces full SDD pipeline.
- **settings-template.json** — Updated for Antigravity Lite positioning.
- **docs/guides/** — Removed references to eliminated commands/skills (/batuta-analyze-prompts, skill-sync, prompt-tracker).
- **academia/** — Updated lessons referencing eliminated hooks and features.

### Principio de diseno

- **Claude Code = Production**: Full SDD pipeline, gates, enforcement, Agent Teams.
- **Antigravity Lite = Exploration**: Brainstorming, quick prototyping, idea validation.
- **Clear handoff path**: "Esta idea esta lista para SDD -> lleva a Claude: /sdd-new {name}".

### Rollback

```bash
git revert <commit-hash>..HEAD  # Revert all v11.2 commits
```
